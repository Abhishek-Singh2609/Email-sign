...
//             disabled={isLoading || filteredEmployees.length === 0} >{isLoading ? "Applying..." : "Apply For All"}</button>
//         <button className="btn btn-remove">Apply to Individual</button>
//         <button className="btn btn-apply">Apply to Groups</button>
//        </div>
//        <div className="search-bar" ><div className="search-container">
...
const SignatureAction = () => {
  ...
  const [error, setError] = useState(null);
  const [signatureApplied, setSignatureApplied] = useState(null);
  const [loadingEmployees, setLoadingEmployees] = useState(false); // Changed initial state to false
  const [removingAll, setRemovingAll] = useState(false);
  const [removingEmployee, setRemovingEmployee] = useState(null);
  const [removingAllBanners, setRemovingAllBanners] = useState(false);
  const [organizationDomain, setOrganizationDomain] = useState(null);
  const [showEmployeeList, setShowEmployeeList] = useState(false); // New state to control visibility
  const navigate = useNavigate();
  ...
  useEffect(() => {
    // Only fetch organization on mount
    fetchOrganization();
  }, []);
